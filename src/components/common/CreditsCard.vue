...
<template>
  <div class="text-zinc-400 p-2 flex flex-col gap-2">
    ...
    <div v-for="credit of getCredits()" class="bg-zinc-800 p-2 rounded">
      <div class="text-amber-500">{{credit.name}}</div>
      <div class="text-sm">Sprites by: {{credit.authors.join(', ')}}</div>
      <div class="text-sm my-2">License: {{credit.licenses.join(', ')}}</div>
      <ul class="text-xs">
        <li v-for="url of credit.urls"><a :href="url" target="_blank" class="hover:text-zinc-500">{{url}}</a></li>
      </ul>
    </div>
  </div>
</template>
